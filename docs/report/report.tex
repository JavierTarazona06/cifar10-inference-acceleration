...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{J5 (I5)} : Consolidation. Mesures finales (3 runs), tableau comparatif, rapport et justification.
\end{enumerate}
...
%TODO: To change when executed, the final distillation
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Reprise en cours} : training réassuré pour 180 epochs supplémentaires.
\end{itemize}
...
% TODO: Mettre à jour après exécution
\subsection{Choix du candidat final}

Au vu des résultats, le candidat final sera l'une des variantes suivantes, en fonction des résultats de distillation :

\begin{itemize}[leftmargin=*]
    \item \textbf{Option A} : MobileNetV3 + distillation (si accuracy $\geq 85\%$) + FP16 + compile = meilleure latence.
    \item \textbf{Option B} : ResNet-18 + compile + FP16 = référence robuste (déjà $\geq 85\%$), mais latence plus élevée.
\end{itemize}

Le choix sera justifié par le tableau final : accuracy vs latence moyenne vs p95.
...
%TODO: To change when executed, the final distillation
\textbf{Modèle final :} Candidat issu de la distillation ou ResNet-18 + compile, avec latence optimisée et accuracy $\geq 85\%$.
